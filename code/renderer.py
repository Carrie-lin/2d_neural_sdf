...
def plot_sdf(sdf_func, device, res_path, name, img_name,epoch_num,mask_path,
             img_size=500, is_net=False, show=False,expid="hhh"):
    ...
    # Generate a heat map
    store_heat_map=plt.figure(figsize=(10,10),dpi=1000)
    ...
    plt.plot(surface_data[:,0], surface_data[:,1], color='w',zorder=5)
    ...
    if new_edge.shape[0]>0:
        plt.scatter(new_edge[:,1],new_edge[:,0],c='k',s=0.5,alpha=1,zorder=10)

    '''
    writer.add_figure(
    'sdf_'+img_name+"_"+str(epoch_num),
    store_heat_map,
    close=False,
    )
    '''
    plt.savefig(f'{res_path}{expid+"_"+str(epoch_num)}.png',dpi=1000)  
    plt.close()
...
def visual_error(testXY,visual_sdf_error,visual_grad_error,img_name,expid,epoch_num):
    ...
    #plt.savefig(f'{TRAINED_PATH}{img_name}_grad.png',dpi=500)  
    plt.clf()
    ...
    # save img
    writer.add_figure(
        'sdfError_'+img_name+"_"+str(epoch_num),
        sdf_error,
        close=False,
        )
    #plt.savefig(f'{TRAINED_PATH}{img_name}_sdf.png',dpi=500)  
    plt.clf()
    plt.close()
...
def visual_gradient(sdf_func,name,epoch_num,expid):
    ...
    gradient_diff=plt.figure(figsize=(11,11))
    #plt.scatter(point_list[:,0],point_list[:,1],s=1)
    #plt.show()
    for i in range(len(testData)):
        if i%80==0:
            ...
            plt.annotate(s="",xytext=(testData[i][0], -testData[i][1]), xy=(testData[i][0]-testData[i][2]*0.05, -testData[i][1]+testData[i][3]*0.05),
                     arrowprops={"arrowstyle": "->"})  # 如果arrowprops中有arrowstyle,就不应该有其他的属性，xy代表的是箭头的位置，xytext代表的是箭头文本的位置。
...
if __name__ == '__main__':
    ...
    #plot_sdf(func, device, res_path=path, name=name, img_name="lgy_test",epoch_num=10,mask_path=MASK_PATH, is_net=net, show=False)    
